...
public class DeviceManager2 {
	...
	public static void sendNoBuffer(WinPcap pcap, MyPaket mp) {
		System.err.println("Send click");
			byte[] byteList = ("111111111111__capturedfrom2"+mp.getType()+mp.getDate()+mp.getData()).getBytes();
//			ByteBuffer byteBuffer = ByteBuffer.wrap(byteList);
			System.arraycopy(ICons.macSource1,0,byteList,0,6); // destination Mac
			System.arraycopy(ICons.macSource2,0,byteList,6,6); // src mac
			byte[] eth = {(byte) 0x88,0x08};
			System.arraycopy(eth,0,byteList,12,2);
			if (pcap.sendPacket(byteList) != Pcap.OK) {
				System.err.println(pcap.getErr());
				System.err.println( "Can not Sended::: "+mp.getType()+mp.getDate()+mp.getData());
			} else {
				System.out.println( "Sended::: "+mp.getType()+mp.getDate()+mp.getData());
			}
//		pcap.close();
	}
	...
	public static void sendBuffer(WinPcap pcap, TableView<MyPaket> xtv1) {
		try {
			pcap = DeviceManager2.getWinPicap(null, 0, 0);
		} catch (IOException e) {
